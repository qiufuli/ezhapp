		...
		methods: {
			...
			tj() {
				...
				let times = setInterval(function() {
					if(self.imgUrls.length != self.imgUrls2.length) {
						...
						setTimeout(function() {
							...
							params.append('createDate', new Date(self.longtime.replace(/\-/g, "/")).getTime());
							params.append('text', '["' + self.morningName1 + '","' + self.morningName2 + '","' + self.middayName1 + '","' + self.middayName2 + '","' + self.eveningName1 + '","' + self.eveningName2 + '"]');
			...
			init() {
				axios.get(address + 'index/api/getRecipes', {
					params: {
						...
						createDate: new Date(this.longtime.replace(/\-/g, "/")).getTime()
					}
				}).then((res) => {
					console.log(res)
