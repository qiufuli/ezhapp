					...
					<scroll :data="datas" class="SP_per">
						<div class="allSelect">
							<div id="quanxuan">
								...
								<label for="all-checked">全选</label>
							</div>
							<div class="erSelect" v-for="(data,index) in datas">
								...
								<div class="erSelectStyle">
									...
									<i @click="goshow(index)" :index="index" ref="goshow"></i>
								</div>
								<transition name="slideshow">
									<div class="trSelect" :id="'trSelect'+index">
										<div class="er" v-for="item in data.listItem">
											<!--遍历子的-->
											<input :id="item.id" type="checkbox" :value="item" v-model="selecteds" /> <label :for="item.id">{{item.name}}</label>
										</div>
									</div>
								</transition>
...
<style scoped>
	...
	.erSelect .er:last-child ,.allSelect .erSelect{
		border-bottom: none ;
	}
	.trSelect {
		display: none;
